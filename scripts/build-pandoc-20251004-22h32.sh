...
#   --out-basename NAME     Base du nom de sortie (def: 00-dossier-projet)
...
OUT_BASENAME="${OUT_BASENAME:-00-dossier-projet}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --out-basename) OUT_BASENAME="${2:-00-dossier-projet}"; OUT_MD="${OUT_BASENAME}.md"; OUT_PDF="${OUT_BASENAME}.pdf"; shift 2;;
...
PAGE_DE_GARDE="$DOCS_DIR/00_PageDeGarde.tex"
BEFORE_BODY_FILES=""
...
mapfile -t SRC_FILES < <(
  find . -maxdepth 1 -type f -regextype posix-extended \
    -regex '^./[0-9]{2}[^/]*\.md$' \
    ! -name '00-dossier-projet.md' \
    ! -name '00_PageDeGarde.md' \
    -printf '%f\n' | sort -V
)
    ...
    try:
        subprocess.run(
            ["mmdc", "-i", str(tmp), "-o", str(png_path),
             "--backgroundColor", "transparent", "-w", "1600"],
